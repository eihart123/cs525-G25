...
function renderDictionary(value: object, formatter: Formatter) {
    ...
    for (const [k, v] of entries) {
        ...
        const suppressKey = isObject(v) && (v as IDiagnostic)[Diagnostic.presentation] === Diagnostic.Presentation.Flag;
        if (!suppressKey) {
...
function valueFor(value: unknown) {
    ...
    const proxied = (value as IDiagnostic)[Diagnostic.value];
    if (proxied) {
        if (proxied === value) {
...
function presentationFor(value: unknown) {
    ...
    if (Diagnostic.presentation in (value as IDiagnostic)) {
        return (value as IDiagnostic)[Diagnostic.presentation];
    }
    const proxied = (value as IDiagnostic)[Diagnostic.value];
