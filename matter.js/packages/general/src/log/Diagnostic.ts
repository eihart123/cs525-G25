 ...
 */
export function Diagnosticf(presentation: Diagnostic.Presentation | Lifecycle.Status, value: unknown): IDiagnostic {
    return {
...
export namespace Diagnostic {
    ...
    export function strong(value: unknown) {
        return Diagnosticf(Diagnostic.Presentation.Strong, value);
    }
    ...
    export function weak(value: unknown) {
        return Diagnosticf(Diagnostic.Presentation.Weak, value);
    }
    ...
    export function flag(value: string) {
        return Diagnosticf(Diagnostic.Presentation.Flag, value);
    }
    ...
    export function via(value: string) {
        if ((value as IDiagnostic)[presentation]) {
            return value;
        }
    ...
    export function added(value: unknown) {
        return Diagnosticf(Diagnostic.Presentation.Added, value);
    }
    ...
    export function deleted(value: unknown) {
        return Diagnosticf(Diagnostic.Presentation.Deleted, value);
    }
    ...
    export function list(value: Iterable<unknown>) {
        return Diagnosticf(Diagnostic.Presentation.List, value);
    }
    ...
    export function squash(...values: unknown[]) {
        return Diagnosticf(Diagnostic.Presentation.Squash, values);
    }

    /**
     * Create a K/V map that presents with formatted keys.
     */
    export function dict(entries: object, suppressUndefinedValues = true): Record<string, unknown> & IDiagnostic {
        const result: any = {
            ...entries,
    ...
    export function lifecycle(status: Lifecycle.Status, value: unknown) {
        return Diagnosticf(status, value);
    }
    ...
    export function lifecycleList(map: Lifecycle.Map<any>) {
        return Object.entries(map).map(([label, status]) => Diagnosticf(status, label));
    }
...
function formatError(error: any, options: { messagePrefix?: string; parentStack?: string[] } = {}) {
    ...
    message = Diagnostic.upgrade(message, Diagnosticf(Diagnostic.Presentation.Error, message));
    ...
    const list: Array<string | IDiagnostic> = [message];
    if (stack === undefined) {
        // Ensure line break in case of no stack
        list.push(Diagnosticf(Diagnostic.Presentation.List, []));
    } else {
        list.push(Diagnosticf(Diagnostic.Presentation.List, stack));
    }
    ...
    for (; typeof cause === "object" && cause !== null; cause = cause.cause) {
        ...
        if ((formatted as IDiagnostic)[Diagnostic.presentation] === Diagnostic.Presentation.List) {
            formatted = (formatted as IDiagnostic)[Diagnostic.value] ?? formatted;
        }
